...
Copy this folder to the machine, then add a non-Steam shortcut per game:
...
Leave it **on**. That is not the obvious answer, so it is worth explaining.
...
Face buttons are drawn in the Switch arrangement and light up **by name**:
press the button marked A and the circle marked A lights, wherever it sits on
your controller. If you would rather match physical position than labels, `L`
...
- That A means A — or the mirrored layout if you prefer
...
- Preflight never wakes or pairs controllers. Pair them in your OS first.
...
Built for a Steam Machine in a living room, aimed squarely at not making three
people wait while you work out whose controller is which.

Currently supports Ryubing (Ryujinx). Dolphin and Eden are planned — most of
